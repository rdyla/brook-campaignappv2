...
export default function ContactImportView() {
  ...
  const [importResults, setImportResults] = useState<ContactImportItemResult[]>([]);
  ...
  async function handleImport() {
    ...
    try {
      // Chunk client-side so each Worker request stays well under wall-time
      // limits even on a 10k-contact upload.
      for (let i = 0; i < contacts.length; i += CONTACT_CHUNK_SIZE) {
        const chunk = contacts.slice(i, i + CONTACT_CHUNK_SIZE);
        ...
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        const data = (await r.json()) as ContactImportResponse;
        // Re-index results to the global contact array so the UI shows the
        // correct row number.
        const reindexed = data.results.map((res) => ({ ...res, index: i + res.index }));
        setImportResults((prev) => [...prev, ...reindexed]);
        setImportDone(i + chunk.length);
      }
      setStage("done");
    } catch (err) {
...
function ImportProgress({
  ...
  const succeeded = results.filter((p) => p.status === "success").length;
  const failed = results.filter((p) => p.status === "failed").length;
  const pct = total > 0 ? Math.round((done / total) * 100) : 0;
  const failedItems = results.filter((p) => p.status === "failed");
